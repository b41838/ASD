...
var getData = function() {
		...
		$.each(localStorage, function( index, element ) {
			...
			var value = localStorage.getItem(index);//(key);
			var obj = JSON.parse(value);		
			var blah = $('#showLS ul').append($('<li/>'));
			$.each(obj, function(index, element) {
               blah
					.append($('<ul/>')
						.append($('<li/>')
							.html(obj[index][0]+" "+obj[index][1])         
					   )		
					);
			});
			//blah.append(createItemLinks(index, blah));
		});
		/*$.each(localStorage, function( index, element ) {
			//var key = localStorage.key(index);
			var value = localStorage.getItem(index);//(key);
			var obj = JSON.parse(value);
			$('#showLS').append($('<article/>').append($('<ul/>')
				.each(obj, function(index, element) {
					$(this).append($('<li/>')
						.html(obj[index][0]+" "+obj[index][1])         
					)
